        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                <v-col style="max-width:140px;">
                    <Number class="attributes-list" label="OwnerId" v-model="value.parameters.ownerId" :editMode="editMode"/>
                </v-col>
                <v-col style="max-width:150px;">
                    <ContentStatus style="margin-top: 0px; margin-left: 10px;" offline label="Status" v-model="value.parameters.status" :editMode="editMode" @change="change"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        ...
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'contents/search/contentList',
                parameters: {}
            },
        }),
        created() {
