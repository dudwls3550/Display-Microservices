        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                <v-col style="max-width:140px;">
                    <String class="attributes-list" label="SearchQuery" v-model="value.parameters.searchQuery" :editMode="editMode"/>
                </v-col>
                <v-col style="max-width:140px;">
                    <String class="attributes-list" label="Category" v-model="value.parameters.category" :editMode="editMode"/>
                </v-col>
                <v-col style="max-width:150px;">
                    <ExternalIntegrationClientId style="margin-top: 0px; margin-left: 10px;" offline label="ExternalIntegrationClientId" v-model="value.parameters.externalIntegrationClientId" :editMode="editMode" @change="change"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        ...
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'externalNotifications/search/externalContentList',
                parameters: {}
            },
        }),
        created() {
