        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                <v-col style="max-width:150px;">
                    <ContentCategory style="margin-top: 0px; margin-left: 10px;" offline label="Category" v-model="value.parameters.category" :editMode="editMode" @change="change"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        ...
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'metadata/search/metadataSummary',
                parameters: {}
            },
        }),
        created() {
