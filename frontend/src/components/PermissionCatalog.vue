        ...
        <v-card-text style="margin-top:-10px;">
            <v-row no-gutters style="margin: 15px 0px -10px -15px;">
                <v-col style="max-width:150px;">
                    <PermissionStatus style="margin-top: 0px; margin-left: 10px;" offline label="StatusFilter" v-model="value.parameters.statusFilter" :editMode="editMode" @change="change"/>
                </v-col>
                <v-col>
...
<script>
    export default {
        ...
        data: () => ({
            editMode: true,
            value: {
                apiPath: 'permissions/search/permissionCatalog',
                parameters: {}
            },
        }),
        created() {
